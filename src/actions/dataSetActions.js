...
export const updateActiveDataSet = ({
  dispatch,
  newDataSet,
  availableGroups,
  activeDataSetName,
...
export const updateFilteredDataSet = (
  dispatch,
  fullDataSet,
  activeFilters,
  availableGroups
  ...
  activeFilters.forEach((filter) => {
    filteredDataSet = filteredDataSet.filter((d) =>
      filter.checkedOptions.includes(
        String(d[`${filter.question} - ${filter.group}`])
      )
    )
  })
  const newStatistics = getStatistics(availableGroups, filteredDataSet)
  dispatch({
    type: UPDATE_FILTERED_DATASET,
    payload: { filteredDataSet, newStatistics },
  })
}
